...
<!-- DK: We should focus here on creating model pipes (and so the same in chapters 10 and 12).  -->

<!-- DK: See a full pipe suggestion from chapter 10 comments. -->
...
Load the libraries needed for this chapter. The ***tidymodels*** framework makes it easy to use functions from various different packages in constructing, fitting, and testing models. 
...
<!-- DK: What should I do with ordered factors? Change them here? Change them in nes itself? If I leave them, will our various models be able to do something with them? -->
...
<!-- DK: Make the data set cleanly, one time. Don't modify in place. Or maybe I should do some of these things with step_() functions in the recipe? -->
...
<!-- BG: I really like how the question comes write before the data splitting. This  may be useful to include for ch. 10? -->
...
<!-- DK: Change MSE to RMSE? -->

How do we know if this is a good model? We can check using cross-validation. Cross-validation helps us to estimate how well (or poorly) the model will do on new data. There are two important characteristics of the MSE we should always keep in mind:
...
<!-- BG: vfold_cv() was used in the last chapter, but no explanation was given. I think we should put the explanation there because it is where the function is first seen. -->
...
<!-- DK: Can't we avoid update()? Seems more complexity than we need. -->
...
<!-- BG: maybe say someting about what mlp() does here? -->
...
<!-- BG: Will there be a conclusion here or is "Wrap-Up" serving that role? -->
