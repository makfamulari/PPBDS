...
<!-- Get rid of infer package, and the associated rep_sample_n() and use the approach from Week 5, Day 1 lecture. -->
...
Question 1: "If we get 16 red beads in a random sample of size 50 taken from an urn with 2,400 (white or red) beads, how many red beads are in the urn?" 

Question 2: "Conditional on our answer to Question 1, what is the probability, using the same earn, that we will draw more than 8 red beads if we use a shovel of size 20?"
...
`r margin_note("For reference, sample_frac() merely re-arranges the rows of the tibble. We use set.seed() to ensure that the beads in our virtual urn are always in the same order. This makes certain that the figures in the book match their written descriptions.")` 
