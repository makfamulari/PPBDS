...
A Preceptor Table is a table with rows and columns for all the data we would (reasonably) like to have. There are three different types of Preceptor Tables that are applicable to our urn example: Actual, Ideal, and Predictive.

The Actual Preceptor Table shows what we actually know. Accordingly, the table is riddled with question marks that the real world saddles us with. The Ideal Preceptor Table is the Preceptor Table with no question marks, and a reasonable number of rows and columns. With an Ideal Preceptor Table, we would calculate the value we want without any need for inference; the solution is a simple matter of arithmetic. Finally, the Predictive Preceptor Table...

MF: Is there a section on the Predictive Preceptor Table for further clarification? I'm having trouble explaining it in words/applying it to my bead example.
