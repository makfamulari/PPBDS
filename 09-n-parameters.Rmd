...
The meanings of $alive\_post_i$ and $\epsilon_i$ are the same as in the first model. Indeed, they are the same throughout these exercises.  $x_{f,i}$ and $x_{m,i}$ are 0/1 variables, just like last chapter. They are *variables* whose value varies across individuals.
...
* $alive\_post_i$ is our outcome variable. In our case, the outcome variable is $alive\_post_i$, the number of days a person is alive after the election. $male_i$ is one of our explanatory variables. If we are predicting the amount of days a male candidate lives after the election, this value will be 1. When we are making this prediction for female candidates, this value will be 0. $alive\_pre_i$ is our other explanatory variable. It is the number of days a candidate has lived before the election. 
...
However, as data scientists, our job is to bring precision to these questions. There are two common sense interpretations. First, we could be curious about the *expected values* for these questions. If we averaged the data for a thousand candidates like these, what would the answer be? Secnd, we could be curious about two specific individuals. How long will they live? *Averages involve questions about parameters. Individuals necessitate questions about predictions.* Those are general claims, violated too often to be firm rules. Yet, they highlight a key point: expected values are less variable than individual predictions.

Consider the "on average" interpretation first. The answer lies in the posterior distributions of the parameters in `fit_gov_5`.

```{r}
fit_gov_5 %>% 
  as_tibble() %>% 
  mutate(WA = `(Intercept)` + 2739.1 + sexMale) %>% 
  mutate (SD = `(Intercept)` - 1842.9 + sexMale) %>% 
  select(WA, SD) %>% 
  ...
  ggplot(aes(days, color = parameters)) +
    geom_histogram(aes(y = after_stat(count/sum(count))),
     ...
     labs(title = "Posterior Predictive Distribution",
         subtitle = "For 62 years-old male candidates from Washington and South Dakota",
         x = "Average Days Lived Post Election",
         y = "Probability") + 
    theme_classic() + 
...
Looking at our posterior above, we can see that the male candidate from Washington is expected to live longer. But how much longer? As in previous chapters, we can manipulate distributions in, more or less, the same way that we manipulate simple numbers. If we want to know the difference between two posterior distributions, we can simply subtract.

<!-- DK: Should we plot subtraction? Doing so highlights the fact that, on average, Washinton is almost guaranteed to live longer. In the prediction, there is a 30% chance that he won't. -->


If we interpret the question as asking for a prediction for a small number of individuals, then we need to use `posterior_predict()`.


First, we create a tibble with the desired input for our model. In our case the tibble has a variable named `sex` which contains two observations both with the same value "Male". There is a `state` variable with the desired states. There is an `alive_pre` set to 10 years worth of days.
...
<!-- DK: More detail, tying this forecast into other forecasts we have created. -->
...
pp %>% 
  ...
  ggplot(aes(diff)) +
    geom_histogram(aes(y = after_stat(count/sum(count))),
                   ...
                   position = "identity") +
    labs(title = "Posterior Probability Distribution", 
         subtitle = "How much longer will Washingto candidate live?",
         x = "Days",
         y = "Probability") +
    theme_classic() + 
    scale_x_continuous(labels = scales::number_format()) +
    scale_y_continuous(labels = scales::percent_format())
```
In words, we would  expect the male candidate from Washington to live longer than the South Dakota candidate. By how much? Well, that number is an unknown parameter. By looking at our posterior above, our best estimate is about 3,000 days, which is roughly 8 years. However, it is important to note that it is possible that the male from South Dakota could outlive the Washington male. In fact, there is a 1/3 chance that he does so.

<!-- DK: Delete this? Also, notice that in this distribution there are `alive_post` values that are negative? Does this make sense? Could a candidate be alive negative days after the election? The answer is no. However, nothing is wrong with the code. The reason for this is because when we assumed epsilon was normally distributed, we allowed numbers to be negative. This is not that important, so let's press on. -->
