...
Consider someone who is about 15,000 days old on Election Day. We have a score or more data points for candidates around that age. A couple died soon after the election. Some of then lived for about 5,000 days after the election. Others lived for more than 20,000 days. The world is filled with variation. But the fitted line tells us that, on average, we would expect a candidate that age to live for about 15,000 days after the election. 
...
<!-- DK: Need a smoother transition back to centering. -->
...
Let's create another model. This time, however, the numerical outcome variable of alive_post as a function of the two explanatory variables we used above: alive_pre and sex. 
...
Consider a new data set, `shaming`, corresponding to an experiment carried out by   titled "Social Pressure and Voter Turnout: Evidence from a Large-Scale Field Experiment". This experiment used several hundred thousand registered voters and a series of mailings to determine the effect of social pressure on voter turnout. 
...
<!-- MF: What should the Preceptor tables look like with these models? Should each model have its own Preceptor Table? -->
...
<!-- MF: I used family = gaussian as I noted it in the lower section drafts, but can't find the purpose online. Also, stan_glm is taking a very long time to run. Is this a concern? -->
...
<!-- MF: Age isn't showing unique values (just one intercept and then `age`). Is that a problem? -->

```{r, cache=TRUE}
model_2 <- stan_glm(data = ch9, 
...
<!-- MF: As it stands, I have all five models in this section. Should the interaction model maybe be in a later, separate section? -->
