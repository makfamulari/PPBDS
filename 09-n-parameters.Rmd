...
There are three unknown parameters, similar to the models we used in Chapter \@ref(three-parameters).

You may recall from middle school algebra that the equation of a line is $y = a + b x$. It is defined by two parameters: $a$ and $b$. The intercept coefficient $a$ is the value of $y$ when $x = 0$. The slope coefficient $b$ for $x$ is the increase in $y$ for every increase of one in $x$. When defining a regression line, we use slightly different notation but the fundamental relationship is the same. 
