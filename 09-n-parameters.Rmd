...
z <- fit_gov_3 %>% as_tibble() %>% mutate(days = `(Intercept)` + sexMale) 
ggplot(z, color = parameter) + geom_density(aes(days)) + geom_density(aes(`(Intercept)`)) +
labs(title = "Posterior Probability Distribution",
         subtitle = "Avg. Days to live After Election for Candidates ",
         x = "Days",
         y = "Probability") + #pivot_longer(cols = sexMale:sexFemale, names_to = "parameter", values_to = "days") 
...
$$ y_i =  \beta_0 + \beta_1 male_i + \beta_2 alive\_pre_i+ \beta_3 alive\_pre_\epsilon_i$$
