...
$$ RMSE = \sqrt{ \frac{1}{N}\sum_{i=1}^{N} (f_{i} - y_i)^2}$$
where $f_i$ is the fitted or predicted value for observation $i$.
...
$$ MAE = \frac{1}{N}\sum_{i=1}^{N} \lvert f_{i} - y_i \rvert$$
...
Third, add a recipe --- which should contain a formula and data --- and then any associated `step_*` functions. (Note that we have simplified this slightly from the example above.)
...
Fourth, assign this pipe to a single workflow object. This object can then be used in several different ways. The best way to name this object is to include the letters `wfl` so as to remember that it is a workflow object.
...
This examination can also be a part of the process of creating a good workflow. Either way, we certainly want to be sure that, for our final model, the predictions on the training data look reasonable.
