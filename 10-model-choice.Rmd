...
We will save discussion for how to work with tidymodels, once selected, until the Temperance sections of Chapters \@ref(continuous-response) and \@ref(discrete-response). The basic intuition is, however, just the same our use of `posterior_epred()` and `posterior_predict()` in Chapter \@ref(n-parameters).
