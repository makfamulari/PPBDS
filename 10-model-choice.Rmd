...
<!-- DK: We could cut 90% of the material in this section. No need to provide an example. -->
...
The column names are standardized across models and do not contain any additional data (such as the type of statistical test). The data previously contained in the row names are now in a column called `terms` and so on. 
...
<!-- DK: This seems confusing, using pipes in the argument to a function. Or is it natural and we should teach students about it earlier? Also, why delete Sale_Price? -->
...
<!-- DK: Need to explain/rewrite this ugly code. -->
...
<!-- DK: Revisit this code! Make it like other code. No add_formula(), I think. -->
...
rf_wflow <- 
  workflow() %>% 
  add_formula(Sale_Price ~ Neighborhood + Gr_Liv_Area + 
                           Year_Built + Bldg_Type) %>% 
  add_model(rf_model) 
```


The `fit_resamples()` function is analogous to `fit()`, but instead of having a `data` argument, `fit_resamples()` has `resamples` which expects an `rset` object like the ones shown above. The possible interfaces to the function are:  
...
<!-- DK: I am confused about keep_pred and what it does in this code. Recall that there used to be stand-alone function defined at the top of the Rmd called keep_pred. I deleted it everywhere I found it and replaced it with control_resamples(save_pred = TRUE). Necessary? -->
