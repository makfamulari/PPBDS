...
**PPBDS.data** includes the data sets for this book. **rsample** includes functions for bootstrapping. **rstanarm**, **broom.mixed**, **gtsummary** and **tidybayes** make it easy to create and display Bayesian models.
...
<!-- DK: Discuss the meaning? -->

<!-- DK: Fun problem set exercise is to pull this data, pivot it, and then create one graphic with all distributions showing. -->

<!-- ##### Model in a table -->

<!-- DK: Could not get this to work. It seems like these gt objects are already text, so there is no easy way to, for example, change the formatting but adding decimals.  -->

<!-- A third way to display the model is to use `tbl_regresssion()` from the **gtsummary** package. (Note that this will often require the **broom.mixed** package as well.) -->
...
<!--                intercept = TRUE, -->
<!--                estimate_fun = ~(decimals = 4)) %>% -->
<!--   as_gt() -->


<!--   tab_header("Average Height of American Men in 2009")  -->


<!-- ``` -->

<!-- See [here](https://cran.r-project.org/web/packages/gtsummary/vignettes/tbl_regression.html) for more details on `tbl_regression()`. -->
