...
**PPBDS.data** includes the data sets for this book. **rsample** includes functions for bootstrapping.
...
Interesting! There are 353 missing values of height in our subset of data. Just using `glimpse()` does not show us that.  Let's filter out the NA's using `drop_na`. This we will delete the rows in which the value of any variable is missing.
...
We have shown you how to use bootstrap sampling to create a posterior distribution for an unknown parameter. Let's use a similar approach to estimate the value of a different unknown parameter: the average height of an American male in 2009. 
...
<!-- DK: We should keep this section but get it to work. -->
...
Fortunately for us, Bayesian models are not hard to execute in R. Sticking to the same filtered male 2009 data, we can reduce all the work we did in the bootstrap method to the `stan_glm()` function which, when fed the right inputs, creates a bayesian generalized linear model to estimate the mean height. This function comes from the **rstanarm** package, which is very useful for Bayesian models in general.

Similarly to the bootstrap method we just went over, this model is taking several resamples of the data when analyzing the distribution of heights in our data to create a posterior distribution. Each resample or simulation is its own model of two parameters (an intercept and sigma). We will get into what these parameters actually mean and how they are represented shortly, but first let's try to run the model
