...
Doing so requires two steps. First, using the `[` operator to turn the object into a simple matrix and then using `as_tibble()` to transform the matrix into a tibble. The resulting object, `pp`, is not easy to work with, both because the variable names are all numbers and because of how big it is.
