...
Note the subset operator --- the `[]` --- at the end of `posterior_predict()`. This is an absurd hack, made necessary by the fact that `posterior_predict()` returns a weird object with class "ppd", which stands for posterior probability distribution. There are some advanced use cases in which this is a useful class of object to work with. But, for the purposes of this book, the "ppd" class is too complex. So, we quickly turn it into a tibble. Doing so requires two steps. First, using the `[]` operator to turn the object into a matrix and then using `as_tibble()` to transform the matrix into a tibble.

The resulting object, `pp`, is not easy to work with. But because the variable names are all numbers and because how big it is.
